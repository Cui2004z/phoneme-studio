...
export function activityRuntime(data, scoreGuess, selectionLine) {
  ...
  const soundLabel = (s) => {
    const p = findSound(s);
    return p ? `${p.label} (as in ${p.example})` : s;
  };
  ...
  document.getElementById("activity-kind").textContent =
    config.type === "wordle" ? "PHONEME WORDLE" : "PHONEME WORD SEARCH";
  ...
  const hint = el(
    ...
    "sound-hint",
    config.hints
      ? "Hover over or focus a phoneme to explore its sound."
      : "Use the phoneme symbols to play.",
  );
  function addHint(button, symbol) {
    button.setAttribute(
      "aria-label",
      `/${symbol}/${config.hints ? " · " + soundLabel(symbol) : ""}`,
    );
    if (config.hints) {
      button.title = `/${symbol}/ · ${soundLabel(symbol)}`;
      button.addEventListener("focus", () => {
        hint.textContent = `/${symbol}/ — ${soundLabel(symbol)}`;
      });
  ...
  if (config.type === "wordle") {
    ...
    [
      ["correct", "✓", "Right place"],
      ["present", "↔", "Different place"],
      ["absent", "−", "Not in word"],
    ].forEach(([status, mark, label]) => {
      const item = el("span");
      item.append(el("i", status, mark), document.createTextNode(label));
      legend.append(item);
    });
    guessPanel.append(legend, message);
    if (config.hints && target.hint)
      guessPanel.append(el("p", "sound-hint", "Word hint: " + target.hint));
    ...
    for (const kind of ["consonant", "vowel"]) {
      ...
      group.append(
        el(
          "p",
          "key-label",
          kind === "vowel" ? "Vowel sounds" : "Consonant sounds",
        ),
      );
      ...
      phonemes
        ...
        .forEach((p) => {
          const button = el("button", "sound-key");
          button.type = "button";
          button.append(el("span", "symbol", p.symbol));
          if (config.labels) button.append(el("small", "", p.label));
          addHint(button, p.symbol);
          button.addEventListener("click", () => add(p.symbol));
          keys.append(button);
        });
      group.append(keys);
    ...
    function render() {
      ...
      for (let row = 0; row < attempts; row++) {
        ...
        for (let col = 0; col < target.phonemes.length; col++) {
          ...
          const tile = el(
            ...
            "tile " +
              state +
              (row === guesses.length && !finished ? " active-row" : ""),
          );
          ...
          tile.setAttribute(
            "aria-label",
            `Guess ${row + 1}, sound ${col + 1}: ${values[col] || "empty"}${state ? ", " + { correct: "right place", present: "in another place", absent: "not in word" }[state] : ""}`,
          );
          board.append(tile);
        }
      }
      progress.textContent = finished
        ? "Activity complete"
        : `Guess ${guesses.length + 1} of ${attempts}`;
      enter.disabled = finished;
      remove.disabled = finished || current.length === 0;
    ...
    function add(s) {
      ...
      if (current.length >= target.phonemes.length) {
        announce(
          "Your guess is ready. Select Check guess, or delete a sound to change it.",
        );
        return;
      }
      current.push(s);
      if (config.hints) hint.textContent = `/${s}/ — ${soundLabel(s)}`;
      announce(
        `${current.length} of ${target.phonemes.length} sounds selected.`,
      );
      render();
    }
    function submit() {
      ...
      if (current.length !== target.phonemes.length) {
        announce(
          `Choose ${target.phonemes.length} sounds before checking your guess.`,
        );
        return;
      }
      const values = [...current],
        feedback = scoreGuess(values, target.phonemes);
      guesses.push({ values, feedback });
      current = [];
      if (feedback.every((s) => s === "correct")) {
        finished = true;
        message.classList.add("success");
        announce(
          `You found it! /${target.phonemes.join("")}/ = ${target.english.toUpperCase()}. ${guesses.length} ${guesses.length === 1 ? "guess" : "guesses"} — well done.`,
        );
      } else if (guesses.length === attempts) {
        finished = true;
        announce(
          `Good practice! The word was /${target.phonemes.join("")}/ = ${target.english.toUpperCase()}. Select Start again to practise it.`,
        );
      } else {
        announce(
          feedback
            .map(
              (s, i) =>
                `/${values[i]}/: ${s === "correct" ? "right place" : s === "present" ? "different place" : "not in word"}`,
            )
            .join(". ") + ". Try your next guess.",
        );
      }
      render();
    }
    ...
    document.addEventListener("keydown", (event) => {
      ...
      if (event.key === "Enter" && !buttonFocused) {
        ...
        const p = phonemes.find(
          (p) =>
            p.symbol ===
            ({ g: "ɡ", r: "ɹ" }[event.key.toLowerCase()] ||
    ...
    restart.addEventListener("click", () => {
      ...
      announce(`A fresh start. Choose ${target.phonemes.length} sounds.`);
      render();
      keyboard.querySelector("button").focus();
    });
    render();
    announce(
      `Ready when you are. Choose ${target.phonemes.length} sounds to begin.`,
    );
  } else {
    let start = null;
    let found = [];
    ...
    board.setAttribute(
      "aria-label",
      "Word search phoneme grid. Drag a straight line or select the first and last tiles. Arrow keys move; Enter or Space selects.",
    );
    ...
    const buttons = grid.map((symbol, index) => {
      ...
      button.setAttribute(
        "aria-label",
        `Row ${Math.floor(index / cols) + 1}, column ${(index % cols) + 1}: /${symbol}/${config.hints ? " " + soundLabel(symbol) : ""}`,
      );
    ...
    showButton.addEventListener("click", () => {
      ...
      announce(
        showAnswers
          ? "Answer guide shown. Find each highlighted word to complete the activity."
          : "Answer guide hidden. Continue your search.",
      );
      render();
    });
    ...
    function render() {
      ...
      words.forEach((word, index) => {
        ...
        item.append(
          el("span", "word-status", complete ? "✓" : "○"),
          el("span", "ipa", "/" + word.phonemes.join("") + "/"),
        );
        if (config.labels || complete)
          item.append(el("small", "", word.english));
        if (config.hints && word.hint)
          item.append(el("small", "word-clue", word.hint));
        if (complete)
          item.setAttribute(
            "aria-label",
            `Found: /${word.phonemes.join("")}/, ${word.english}`,
          );
        list.append(item);
      });
    }
    function select(index) {
      ...
      if (start === null) {
        start = index;
        if (config.hints)
          hint.textContent = `/${grid[index]}/ — ${soundLabel(grid[index])}`;
        announce(
          `First sound /${grid[index]}/ selected. Now select the last tile. Select the same tile to cancel.`,
        );
        render();
        return;
      }
      ...
      const match = words.findIndex(
        (word) =>
          word.phonemes.join("|") === sounds ||
          word.phonemes.join("|") === reverse,
      );
      if (match < 0) {
        announce(
          "That is not one of the listed words. Try another straight line.",
        );
        ...
        announce(
          `Found /${words[match].phonemes.join("")}/ = ${words[match].english.toUpperCase()}! ${words.length - found.length} words to go.`,
        );
        if (found.length === words.length) {
          ...
          announce(
            `All ${words.length} words found! ` +
              words
                .map((word) => `/${word.phonemes.join("")}/ = ${word.english}`)
                .join(", ") +
              ". Well done.",
          );
        }
    ...
    restart.addEventListener("click", () => {
      ...
      message.classList.remove("success");
      announce(
        "A fresh search. Drag along a word or select its first and last tiles.",
      );
      render();
      buttons[0].focus();
    });
